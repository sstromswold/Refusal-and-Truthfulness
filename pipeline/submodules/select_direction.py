...
#    steering_refusal_scores = torch.zeros((n_pos, n_layer), device=model_base.model.device, dtype=torch.float32)
    ...
    kl_values = ablation_kl_div_scores.detach().cpu().numpy()
    steering_values = steering_refusal_scores.detach().cpu().numpy()
    valid_mask = ~np.isnan(kl_values)
    kl_values = kl_values[valid_mask]
    steering_values = steering_values[valid_mask]
    
    print("KL mean/std:", kl_values.mean(), kl_values.std())
    print("Steering mean/std:", steering_values.mean(), steering_values.std())

    print("Candidate directions:", n_pos * n_layer)
    print("KL > threshold:", (ablation_kl_div_scores > kl_threshold).sum().item())
    print("Any KL NaN:", torch.isnan(ablation_kl_div_scores).any().item())

    print("Steering < threshold:", (steering_refusal_scores < induce_refusal_threshold).sum().item())
    print("Layers pruned:", int(n_layer * prune_layer_percentage))
