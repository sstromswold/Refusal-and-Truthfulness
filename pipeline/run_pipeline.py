...
def load_and_sample_datasets(cfg):
    ...
    """
    random.seed(42)
    impossible_train = random.sample(load_dataset_split(harmtype='impossible', split='train', instructions_only=True), cfg.n_train)
    possible_train = random.sample(load_dataset_split(harmtype='possible', split='train', instructions_only=True), cfg.n_train)
    impossible_val = random.sample(load_dataset_split(harmtype='impossible', split='val', instructions_only=True), cfg.n_val)
    possible_val = random.sample(load_dataset_split(harmtype='possible', split='val', instructions_only=True), cfg.n_val)
    return impossible_train, possible_train, impossible_val, possible_val
...
def generate_and_save_completions_for_dataset(cfg, model_base, fwd_pre_hooks, fwd_hooks, intervention_label, dataset):
...
#        dataset = json.load(f)    
    ...
    with open(f'{cfg.artifact_path()}/completions/truthful_{intervention_label}_completions.json', "w") as f:
...
def evaluate_completions_and_save_results_for_dataset(cfg, intervention_label, dataset_name, eval_methodologies):
    ...
    with open(os.path.join(cfg.artifact_path(), f'completions/truthful_{intervention_label}_completions.json'), 'r') as f:
        completions = json.load(f)

    evaluation = evaluate_jailbreak(
        completions=completions,
        methodologies=eval_methodologies,
        evaluation_path=os.path.join(cfg.artifact_path(), "completions", f"truth_{intervention_label}_evaluations.json"),
    )

    with open(f'{cfg.artifact_path()}/completions/truth_{intervention_label}_evaluations.json', "w") as f:
...
#    print("Activation Addition Loss")
...
def run_pipeline(model_path):
    ...
    model_base = construct_model_base(cfg.model_path)
#    possible_train = "/home/common/neural-safety-net/refusal_direction/dataset/processed/possible_train.json"
#    possible_val = "/home/common/neural-safety-net/refusal_direction/dataset/processed/possible_val.json"
#    impossible_train = "/home/common/neural-safety-net/refusal_direction/dataset/processed/impossible_train.json"
#    impossible_val = "/home/common/neural-safety-net/refusal_direction/dataset/processed/impossible_val.json"
    random.seed(42)
    impossible_eval_data = "/home/common/neural-safety-net/refusal_direction/dataset/splits/impossible_test.json"
    possible_eval_data = "/home/common/neural-safety-net/refusal_direction/dataset/splits/possible_test.json"
    with open(impossible_eval_data, "r") as f:
        impossible_eval_data_loaded = json.load(f) 
    with open(possible_eval_data, "r") as f:
        possible_eval_data_loaded = json.load(f)

    impossible_eval_data_sample = random.sample(impossible_eval_data_loaded, 100)    
    possible_eval_data_sample = random.sample(possible_eval_data_loaded, 100)
    # Load and sample datasets
    impossible_train_sample, possible_train_sample, impossible_val_sample, possible_val_sample = load_and_sample_datasets(cfg)

    # 1. Generate candidate refusal directions
    candidate_directions = generate_and_save_candidate_directions(cfg, model_base, impossible_train_sample, possible_train_sample)
    
    # 2. Select the most effective refusal direction
    pos, layer, direction = select_and_save_direction(cfg, model_base, impossible_val_sample, possible_val_sample, candidate_directions)
    ...
    # 3a. Generate and save completions on harmful evaluation datasets
    #for dataset_name in cfg.evaluation_datasets:
    generate_and_save_completions_for_dataset(cfg, model_base, baseline_fwd_pre_hooks, baseline_fwd_hooks, 'baseline_impossible', impossible_eval_data_sample)
    generate_and_save_completions_for_dataset(cfg, model_base, ablation_fwd_pre_hooks, ablation_fwd_hooks, 'ablation_impossible', impossible_eval_data_sample)
    generate_and_save_completions_for_dataset(cfg, model_base, actadd_fwd_pre_hooks, actadd_fwd_hooks, 'actadd_impossible', impossible_eval_data_sample)

    # 3b. Evaluate completions and save results on harmful evaluation datasets
    evaluate_completions_and_save_results_for_dataset(cfg, 'baseline_impossible', impossible_eval_data_sample, eval_methodologies=cfg.truthful_eval_methodologies)
    evaluate_completions_and_save_results_for_dataset(cfg, 'ablation_impossible', impossible_eval_data_sample, eval_methodologies=cfg.truthful_eval_methodologies)
    evaluate_completions_and_save_results_for_dataset(cfg, 'actadd_impossible', impossible_eval_data_sample, eval_methodologies=cfg.truthful_eval_methodologies)

    # 4a. Generate and save completions on harmless evaluation dataset
#    harmless_test = random.sample(load_dataset_split(harmtype='harmless', split='test'), cfg.n_test)

    generate_and_save_completions_for_dataset(cfg, model_base, baseline_fwd_pre_hooks, baseline_fwd_hooks, 'baseline_possible', possible_eval_data_sample)

    actadd_refusal_pre_hooks, actadd_refusal_hooks = [(model_base.model_block_modules[layer], get_activation_addition_input_pre_hook(vector=direction, coeff=+1.0))], []
    generate_and_save_completions_for_dataset(cfg, model_base, actadd_refusal_pre_hooks, actadd_refusal_hooks, 'actadd_possible', possible_eval_data_sample)

    # 4b. Evaluate completions and save results on harmless evaluation dataset
    evaluate_completions_and_save_results_for_dataset(cfg, 'baseline_possible', possible_eval_data_sample, eval_methodologies=cfg.truthful_eval_methodologies)
    evaluate_completions_and_save_results_for_dataset(cfg, 'actadd_possible', possible_eval_data_sample, eval_methodologies=cfg.truthful_eval_methodologies)

    # 5. Evaluate loss on harmless datasets
    print("Baseline Loss")
    evaluate_loss_for_datasets(cfg, model_base, baseline_fwd_pre_hooks, baseline_fwd_hooks, 'baseline_possible')
    print("Ablation Loss")
    evaluate_loss_for_datasets(cfg, model_base, ablation_fwd_pre_hooks, ablation_fwd_hooks, 'ablation_possible')
    print("Activation Addition Loss")
    evaluate_loss_for_datasets(cfg, model_base, actadd_fwd_pre_hooks, actadd_fwd_hooks, 'actadd_possible')
